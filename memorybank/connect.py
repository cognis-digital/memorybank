"""Native cognis-connect emit for memorybank — forward findings to any platform.

Maps memorybank's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    memorybank ... --format json | memorybank-emit --to stix
    memorybank-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a memorybank record toward the Finding contract before normalize().
